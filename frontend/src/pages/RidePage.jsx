...
import { Car, Plus, Users, MapPin, Clock, Zap, ArrowLeft, Navigation, Fuel, Wind } from 'lucide-react';
...
const StatPill = ({ icon, label, value, color }) => (
  <motion.div
    ...
    style={{
      ...
      boxShadow: `0 0 20px ${color}11, inset 0 1px 0 rgba(255,255,255,0.05)`,
      cursor: 'default',
    }}
  >
    <div style={{
    ...
    >
      <div style={{
        ...
        onMouseEnter={e => {
          ...
          e.currentTarget.style.boxShadow = '0 0 60px rgba(251,191,36,0.08), 0 0 40px rgba(0,0,0,0.4), inset 0 1px 0 rgba(255,255,255,0.06)';
        }}
        onMouseLeave={e => {
          e.currentTarget.style.borderColor = 'rgba(255,255,255,0.08)';
          e.currentTarget.style.boxShadow = '0 0 40px rgba(0,0,0,0.4), inset 0 1px 0 rgba(255,255,255,0.06)';
        }}
      >
        {/* Glow spot */}
        <div style={{
          ...
          filter: 'blur(60px)', opacity: 0.06, pointerEvents: 'none',
        }} />
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            <motion.div
              animate={{ boxShadow: [`0 0 8px ${statusColor}66`, `0 0 16px ${statusColor}99`, `0 0 8px ${statusColor}66`] }}
              transition={{ duration: 2, repeat: Infinity }}
              style={{
                width: 8, height: 8, borderRadius: '50%', background: statusColor,
              }}
            />
            <span style={{ fontSize: '0.68rem', textTransform: 'uppercase', letterSpacing: '0.1em', color: statusColor, fontWeight: 700 }}>
              {ride.status}
            </span>
          </div>
          {ride.matchScore > 0 && (
            <span style={{
              ...
              background: ride.matchScore > 70 ? 'rgba(34,197,94,0.15)' : 'rgba(245,158,11,0.15)',
              color: ride.matchScore > 70 ? '#22c55e' : '#f59e0b',
              border: `1px solid ${ride.matchScore > 70 ? '#22c55e44' : '#f59e0b44'}`,
        ...
        <div style={{ marginBottom: '1rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.3rem' }}>
            ...
            <span style={{ fontSize: '1rem', fontWeight: 700, color: '#fff', fontFamily: "'Bebas Neue', cursive", letterSpacing: '0.04em' }}>
              {ride.origin}
            </span>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', paddingLeft: '1.5rem', marginBottom: '0.25rem' }}>
          ...
          {ride.matchReason && (
            <p style={{ fontSize: '0.72rem', color: 'rgba(255,255,255,0.4)', marginTop: '0.4rem', paddingLeft: '1.5rem' }}>{ride.matchReason}</p>
          )}
        </div>
        ...
        <div style={{ display: 'flex', gap: '0.5rem' }}>
          {ride.status === 'open' && (
            <motion.button
              ...
              style={{
                flex: 1, padding: '0.65rem', borderRadius: 12, border: 'none', cursor: 'pointer',
          ...
          {ride.creator?._id === localStorage.getItem('userId') && !isCompleted && (
            <motion.button
              whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}
              onClick={() => onComplete(ride._id)}
              style={{
                flex: 1, padding: '0.65rem', borderRadius: 12, border: '1px solid rgba(34,197,94,0.4)', cursor: 'pointer',
            ...
            >
              <Zap size={14} /> Complete & Split
            </motion.button>
          )}
          {isFull && !isCompleted && (
            <div style={{ flex: 1, textAlign: 'center', padding: '0.65rem', color: '#f59e0b', fontSize: '0.8rem', fontWeight: 700, fontFamily: "'Bebas Neue', cursive", letterSpacing: '0.08em' }}>
              🔒 FULL
            </div>
          )}
          {isCompleted && (
            <div style={{ flex: 1, textAlign: 'center', padding: '0.65rem', color: 'rgba(255,255,255,0.3)', fontSize: '0.8rem', fontFamily: "'Bebas Neue', cursive", letterSpacing: '0.08em' }}>
              ✓ COMPLETED
            </div>
          )}
        </div>
...
const RidePage = () => {
  ...
  const handleCreate = async () => {
    try {
      await axios.post(`${API}/api/rides/create`, {
        ...newRide, totalSeats: Number(newRide.totalSeats), estimatedFare: Number(newRide.estimatedFare) || 0,
  ...
  const handleComplete = async (rideId) => {
    try {
      const { data } = await axios.post(`${API}/api/rides/complete`, { rideId }, { headers: { Authorization: `Bearer ${token}` } });
        ...
        <div style={{ maxWidth: 960, margin: '0 auto', padding: '2rem 1.5rem', position: 'relative', zIndex: 1 }}>
          ...
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}
            ...
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.4rem' }}>
                <motion.div
                  ...
                  style={{ fontSize: '2.5rem', filter: 'drop-shadow(0 0 12px #fbbf2488)' }}
                >🚗</motion.div>
              ...
              <p style={{ color: 'rgba(255,255,255,0.35)', fontSize: '0.88rem', letterSpacing: '0.06em', textTransform: 'uppercase' }}>
                ⚡ AI-powered campus ride matching
              </p>
            </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
              ...
              <motion.button
                ...
                style={{
                  background: showCreate ? 'rgba(255,255,255,0.06)' : 'linear-gradient(135deg, #fbbf24, #f97316)',
                  border: showCreate ? '1px solid rgba(255,255,255,0.1)' : 'none',
                  borderRadius: 14, padding: '0.75rem 1.4rem',
                  color: showCreate ? 'rgba(255,255,255,0.6)' : '#000',
                  fontWeight: 800, fontSize: '0.85rem', cursor: 'pointer',
                  fontFamily: "'Bebas Neue', cursive", letterSpacing: '0.1em',
                  boxShadow: showCreate ? 'none' : '0 4px 24px rgba(251,191,36,0.4)',
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
            ...
            <div style={{ flex: 1, minWidth: 160, position: 'relative' }}>
              <Navigation size={13} style={{ position: 'absolute', left: '0.9rem', top: '50%', transform: 'translateY(-50%)', color: '#fbbf24', filter: 'drop-shadow(0 0 4px #fbbf24)', pointerEvents: 'none' }} />
              <input className="ride-input" value={searchOrigin} onChange={e => setSearchOrigin(e.target.value)}
                placeholder="From (e.g. Main Gate)" style={{ paddingLeft: '2.2rem' }} />
            </div>
            <div style={{ flex: 1, minWidth: 160, position: 'relative' }}>
              <MapPin size={13} style={{ position: 'absolute', left: '0.9rem', top: '50%', transform: 'translateY(-50%)', color: '#f97316', filter: 'drop-shadow(0 0 4px #f97316)', pointerEvents: 'none' }} />
              <input className="ride-input" value={searchDest} onChange={e => setSearchDest(e.target.value)}
                placeholder="To (e.g. City Mall)" style={{ paddingLeft: '2.2rem' }} />
            </div>
            <motion.button
              ...
              style={{
                ...
                border: 'none', borderRadius: 12, padding: '0.75rem 1.2rem',
            ...
            {showCreate && (
              ...
              >
                <div style={{
                  ...
                  <h2 style={{ fontFamily: "'Bebas Neue', cursive", fontSize: '1.4rem', color: '#fbbf24', letterSpacing: '0.08em', marginBottom: '1.2rem', textShadow: '0 0 12px #fbbf2444', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                    <Wind size={16} /> POST A NEW RIDE
                  </h2>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '0.75rem', marginBottom: '1rem' }}>
                    {[
                      ['origin', 'From (pickup)', 'text'],
                      ['destination', 'To (drop)', 'text'],
                      ['departureTime', 'Departure time', 'datetime-local'],
                      ['totalSeats', 'Total seats', 'number'],
                      ['estimatedFare', 'Est. fare (₹)', 'number'],
                      ...
                      <div key={key}>
                        ...
                        <input className="ride-input" type={type} placeholder={ph}
                          value={newRide[key]}
                          onChange={e => setNewRide(p => ({ ...p, [key]: e.target.value }))} />
                      </div>
                    ))}
                  </div>
                  <motion.button
                    ...
                    style={{
                      ...
                      fontFamily: "'Bebas Neue', cursive", letterSpacing: '0.12em',
          ...
          {loading ? (
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ textAlign: 'center', padding: '5rem 0' }}>
              <motion.div animate={{ y: [0, -10, 0] }} transition={{ duration: 3, repeat: Infinity }} style={{ fontSize: '4rem', marginBottom: '1rem', filter: 'drop-shadow(0 0 20px rgba(251,191,36,0.3))' }}>🛣️</motion.div>
          ...
          {completionData && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              ...
              <motion.div
                ...
                style={{
                  background: 'linear-gradient(135deg, rgba(15,17,25,0.98), rgba(20,22,32,0.98))',
                  border: '1px solid rgba(251,191,36,0.2)',
                  borderRadius: 24, padding: '2rem', maxWidth: 440, width: '100%',
                  boxShadow: '0 0 80px rgba(251,191,36,0.1), 0 40px 80px rgba(0,0,0,0.6)',
                }}
              >
                <div style={{ textAlign: 'center', marginBottom: '1.5rem' }}>
                  <motion.div animate={{ scale: [1, 1.1, 1] }} transition={{ duration: 2, repeat: Infinity }} style={{ fontSize: '3rem', marginBottom: '0.75rem', filter: 'drop-shadow(0 0 16px #fbbf2466)' }}>🤖</motion.div>
                  ...
                  {completionData.passengers?.map((p, i) => (
                    <motion.div key={p._id}
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
                        ...
                        <span style={{ color: '#fff', fontWeight: 500, fontSize: '0.9rem' }}>{p.name}</span>
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
                        <span style={{ color: '#fbbf24', fontWeight: 800, fontFamily: "'Bebas Neue', cursive", fontSize: '1.1rem', textShadow: '0 0 8px #fbbf2466' }}>₹{completionData.farePerPerson}</span>
                        {p.defaultUpi && (
                          <a href={`upi://pay?pa=${p.defaultUpi}&am=${completionData.farePerPerson}`}
                            style={{ fontSize: '0.7rem', background: 'rgba(34,197,94,0.15)', border: '1px solid rgba(34,197,94,0.3)', color: '#22c55e', padding: '0.25rem 0.6rem', borderRadius: 8, textDecoration: 'none', fontWeight: 600 }}>
                            UPI
                          </a>
                        )}
                      </div>
                ...
                <motion.button
                  ...
                  style={{ width: '100%', padding: '0.9rem', background: 'linear-gradient(135deg, #fbbf24, #f97316)', border: 'none', borderRadius: 14, cursor: 'pointer', color: '#000', fontWeight: 800, fontFamily: "'Bebas Neue', cursive", letterSpacing: '0.1em', fontSize: '1rem', boxShadow: '0 4px 20px rgba(251,191,36,0.4)' }}>
                  🏁 DONE
                </motion.button>
              </motion.div>
            </motion.div>
